...
<html>
<head>
<title>Captcha-Bank Image Test Script</title>
</head>
<body>
<h2>Captcha-Bank Image Test Script</h2>
<p>This script will test your PHP installation to see if Captcha Bank will run on your server.</p>
<ul>
...
<br />No FreeType support.  You cannot use Captcha Bank 3.0, but can use 2.0 with gd fonts.
<?php endif; ?></li>
...
SQLite is available.  If you choose to use it, Captcha Bank can support users who do not accept cookies.
<?php else: ?>
No SQLite support. Captcha Bank will work but your visitors must accept cookies.
<?php endif; ?>
</li>
<li><strong>MySQL Support:</strong>
<?php print_status(extension_loaded('pdo_mysql')); ?><br />
<?php if (extension_loaded('pdo_mysql')): ?>
MySQL is available.  If you choose to use it, Captcha Bank can support users who do not accept cookies by storing codes in MySQL.
<?php else: ?>
No MySQL support. Captcha Bank will work but your visitors must accept cookies.
<?php endif; ?>
</li>
...
<?php if ($GLOBALS['session_start_error'] != null): ?>
There is a warning, but otherwise you meet the requirements for using Captcha Bank.
<?php else: ?>
Your server meets the requirements for using Captcha Bank!
<?php endif; ?>
<br /><br />
<img src="<?php echo CAPTCHA_BK_PLUGIN_URL . '/captcha_bank_test.php?testimage=1' ?>" alt="Test Image" align="bottom" />
<?php else: ?>
Based on the requirements, you do not have what it takes to run Captcha Bank :(
<?php endif; ?>
</body>
</html>