...
if ( ! function_exists( "captcha_bank_login_check" ) )
{
	function captcha_bank_login_check($user, $password) 
	{
		...
		if($err)
		{
			...
			if($err == "empty")
			{
				$error = new WP_Error( "captcha_wrong", "<strong>ERROR: </strong>Captcha Code is empty. Please enter captcha code.");
			}
			else if($err == "invalid")
			{
				$error = new WP_Error( "captcha_wrong", "<strong>ERROR: </strong>The Captcha Code does not match. Please Try Again.");
			}
			return $error;
		}
		return $user;
...
if ( ! function_exists( "captcha_bank_register_post" ) )
{
	function captcha_bank_register_post($user,$email,$errors)
	{
		...
		if($err)
		{
			if($err == "empty")
			{
				$errors->add("captcha_error", "<strong>ERROR: </strong>Captcha Code is empty. Please enter captcha code.");
			}
			else if($err == "invalid")
			{
				$errors->add("captcha_error", "<strong>ERROR: </strong>The Captcha Code does not match. Please Try Again.");
			}
			return $errors;
		}
	}
	...
	function captcha_bank_register_post_mu($errors)
	{
		if ($_POST["stage"] == "validate-user-signup")
		{
			...
			if($err)
			{
				if($err == "empty")
				{
					$errors['errors']->add("captcha", "<strong>ERROR: </strong>Captcha Code is empty. Please enter captcha code.");
				}
				else if($err == "invalid")
				{
					$errors["errors"]->add("captcha", "<strong>ERROR: </strong>The Captcha Code does not match. Please Try Again.");
				}
				return $errors;
			}
		}
